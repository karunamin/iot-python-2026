...
- 설치
    ...
    - Advancd Options에서 Install Python 3.12 for all users 활성화
    - Install 시작
    - 설치 후
...
- 웹 코딩 환경
    ...
    - Visul Studio Code와 동일한 화면으로 변경
...
11. 라이브러리 사용 계속 - [소스](./day03/ex11_outpackage.py)
...
14. 가상환경(Virtual Environment)
    - 프로젝트 마다 파이썬 환경을 따로 사용하기 위해 만드어진 개념
...
15. 객체지향 - [소스1](./day03/ex15_oop.py) ~ [소스4](./day03/ex18_encapsule.py)
    ...
    - C++과 달리 new 안씀 변수등 선언 제약사항이 많이 없음
    - 클래스 내의 모든 함수의 첫번째 파라미터는 'self'로 시작, C++의 this와 동일
    - 호출시에는 self를 사용 X 
    - 파이써느이 철학 : `막지 말고, 알아서 지켜라`
...
16. 예외처리 - [소스](./day03/ex19_exception.py)
    ...
    - except 를 여러번 쓸 수 있으나, `except Exception as e` 하나로 통일해도 무방
    - 예외처리가 발생하면 처리속도가 늦어짐

### 파일 입출력
- 인코딩
    - EUC-KR : 2바이트 한글 완성형 인코딩. CP949 동일한 의미
    - UTF-8 : 1바이트 영문(ASCII호환), 3바이트 한글, 4바이트 이모지 등 특수문자 최대 4바이트사용 국제어표준
...
- JSON
    - JavaScript Object Notation : 자바스크립트에서 데이터를 사용하기 위해 만든 표기 방법
    - 딕셔너리를 텍스트화
    - 데이터를 네트워크로 전달할때 가장 효율적인 파일형식
    - XML을 대체하는 기술
    - 저장된 json 파일을 사용 또는 OpenAPI
...
- 주피터 노트북 단축키 
    - a : [선택모드]현재 셸 위에 코드셸 추가됨
    - b : [선택모드]현재 셸 아래에 코드셸 추가됨
    - enter : [선택모드]현재 셸 편집모드로 진입(커서 깜빡임 확인)
    - Ctrl + enter : [편집모드]마크다운셸은 빠져나오기, 코드셸을 실행
    - l : [선택모드]셸 선택모드에서 라인번호 표시 토글
    - dd : [선택모드]셸 선택모드에서 셸 삭제
    - art + 위아래 방향키 : [선택모드]셸 위치변경
    - c : [선택모드]셸 선택모드에서 셸 복사
    - v : [선택모드]셸 선택모드ㅔ에서 셸 복사하기

- 사용처
    - 웹상에서 통작하므로 많은 서비스를 지원
    - [Github Codespace](https://github.com/features/codespaces) - 기본 리포지토리와 연결 지원(무료일 경우 한달 140시간)
    - [Google Colab](https://colab.google.com/) - 구글에서 지원하는 노트북서비스. 구글드라이브와 연결. 90분 연결 무료, 기능 제약적
...
- 데이터분석
    - 인사이트(Insight) : 특정한 맥악 속에서 특정 원인이나 효과를 이해하는 것
    - 방대한 데이터 속에서 패턴이나 인사이트(통찰)를 도출, `합리적인 의사결정`, `고객 행동 예측`, ` 운영 효율화`, `신규 비즈니스 기회 추출`  등을 하는 핵심도구
        - 데이터 기반의 의사결정 가능
        - 고객 이해도 증가
        - 운영 효율성 및 비용 절감
        - 트렌트 파악 및 경쟁력 강화
...
#### 기초 통계
    ...
    - 정규분포(Nomal Distribution) - 현재의 값이 정상범위인지 판단할때. 퍼진정도를 그래프로, 종모양
    - `이상치(Outlier)` - 튀는 값
...
- 데이터 전처리
    ...
    - 인코딩
        - 문자를 숫자로 변환
        - 예. mail, femail 은 분석불가 -> 0, 1 (수, 분포,.. ) 수치적인 통계 가능
        - One-Hot Encoding, mail[1, 0, 0], femail[0, 1, 0], child[0, 0, 1]
...
### 가상환경 실행
- 생성한 가산환경 내에 Script 폴더 안, Activtate.ps1 실행해야 가상환경 준비
...
- Machine Learning - 데이터로 규칙을 컴퓨터 스스로 찾아내는 알고리즘
- Deep Learning - 인간과 유사한 **인공 신경망**으로 패턴을 학습, 음성인식 , 번역
